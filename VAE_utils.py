...
class SpatialVeloDecoder(VeloDecoder):
    def __init__(self, latent_dim, hidden_dims, output_dim, linear_decoder=False, activation="relu", dropout=0, dtype=torch.float64, norm="batchnorm"):
        super(SpatialVeloDecoder, self).__init__(latent_dim, hidden_dims, output_dim, linear_decoder=linear_decoder, activation=activation, dropout=dropout, dtype=dtype, norm=norm)
        self.linear_decoder = linear_decoder
        self.output_dim = output_dim
...
class MultiSpatialVeloDecoder(SpatialVeloDecoder):
    def __init__(self, latent_dim, hidden_dims, output_dim, linear_decoder=False, activation="relu", dropout=0, dtype=torch.float64, norm="batchnorm", n_states=10, n_t_decoder=6, alpha=None, beta=None, gamma=None):
        super(MultiSpatialVeloDecoder, self).__init__(latent_dim, hidden_dims, output_dim, linear_decoder=linear_decoder, activation=activation, dropout=dropout, dtype=dtype, norm=norm, alpha=alpha, beta=beta, gamma=gamma)
