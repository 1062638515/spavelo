...
#        self.train()
    ...
    def get_encoder_latent(self, adata, spatial_layer="spatial", spliced_layer="Ms", unspliced_layer="Mu", 
                             ...
                             return_numpy=None):
        """Returns cells by genes by latent dim.
        ...
        Returns
        -------
        If `n_samples` > 1 and `return_mean` is False, then the shape is `(samples, cells, latent)`.
        Otherwise, shape is `(cells, latent)`. In this case, return type is :class:`~pandas.DataFrame` unless `return_numpy` is True.
        """
    ...
    def get_state_assignment(self, adata, spatial_layer="spatial", spliced_layer="Ms", unspliced_layer="Mu", 
        ...
        Returns
        -------
        If hard_assignment=True, return state of each gene in each cell. Others return state probability.
        """
        ...
        states = np.concatenate(states, axis=0) if return_mean else np.concatenate(states, axis=1)
        state_cats = [
            "induction",
            "induction_steady",
            "repression",
    ...
    def get_rho(self, adata, spatial_layer="spatial", spliced_layer="Ms", unspliced_layer="Mu", 
                indices=None, gene_list=None, hard_assignment=False, n_samples=25, batch_size=None, return_mean=True,
                return_numpy=None):
        """Returns rho with shape (cells, genes).
